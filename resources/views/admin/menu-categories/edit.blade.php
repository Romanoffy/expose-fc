    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="mb-0 text-white" style="color: var(--text-secondary);">Ubah data kegiatan/event</p>
            </div>
                    ...
                    <div class="card-header" style="background: var(--glass-bg); border-bottom: 1px solid var(--glass-border); border-radius: 16px 16px 0 0;">
                        <div class="d-flex align-items-center gap-3">
                            <div class="rounded-circle p-2" style="background: var(--primary-bg);">
                                <i class="fas fa-edit" style="color: var(--primary-light); font-size: 16px;"></i>
                            </div>
                            <div>
                                <h5 class="mb-0" style="color: var(--text-primary); font-weight: 600;">Informasi Event</h5>
                                <small class="text-white">Ubah detail event</small>
                            </div>
                        </div>
                    </div>

                    <div class="card-body p-4">
                        <form action="/admin/menu-categories/{{ $menuCategory->id }}" method="POST" id="eventForm">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select name="category" id="category" 
                                        ...
                                        @foreach($categoryLabels as $key => $label)
                                            <option value="{{ $key }}" {{ old('category', $menuCategory->category) == $key ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="event_name" id="event_name" 
                                           value="{{ old('event_name', $menuCategory->event_name) }}"
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea name="description" id="description" 
                                              ...
                                              style="background: var(--glass-bg); border: 1px solid var(--glass-border); border-radius: 10px; color: var(--text-primary); padding: 12px 16px;"
                                              rows="4"
                                              placeholder="Masukkan deskripsi event (opsional)...">{{ old('description', $menuCategory->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card" style="background: var(--dark-card); border: 1px solid var(--glass-border); border-radius: 16px;">
                    ...
                    <div class="card-body p-4 text-center">
                        ...
                        <div id="preview-info">
                            <h5 id="preview-event" class="mb-2" style="color: var(--text-primary);">{{ $menuCategory->event_name }}</h5>
                            @php
                                $colorClass = [
                                    'internal' => 'bg-primary',
                                    'external' => 'bg-success',
                                    'friendly' => 'bg-warning',
                                ][$menuCategory->category] ?? 'bg-secondary';
                            @endphp
                            <span id="preview-category" class="badge {{ $colorClass }} mb-3" style="font-size: 12px;">
                                <i class="fas fa-layer-group me-1"></i>{{ $menuCategory->category_label }}
                            </span>
                            <p id="preview-description" class="small mb-3 text-white">
                                {{ $menuCategory->description ?: 'Deskripsi akan muncul di sini' }}
                            </p>
                        </div>
                    </div>
                </div>
